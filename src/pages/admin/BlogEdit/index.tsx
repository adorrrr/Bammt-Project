...
export const AdminBlogEditPage: React.FC = () => {
  ...
  const handleUpdateListItem = (blockIndex: number, itemIndex: number, val: string) => {
    const updated = [...blocks];
    const target = updated[blockIndex];
    if (target.type === 'list') {
      target.items[itemIndex] = val;
      setBlocks(updated);
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEditing && slug) {
      adminService.updateBlogPost(slug, postData);
    } else {
      adminService.createBlogPost(postData);
    }
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900 font-serifBangla">
              {isEditing ? 'আর্টিকেল সম্পাদনা করুন' : 'নতুন ব্লগ আর্টিকেল প্রকাশ'}
            </h1>
            <p className="text-xs text-slate-500">
              {isEditing ? `স্ল্যাগ: /blog/${slug}` : 'জ্ঞানভিত্তিক নিবন্ধ তৈরি করে পাবলিক ব্লগে শেয়ার করুন'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-white border border-slate-200 text-slate-700 hover:text-slate-900 text-xs font-semibold flex items-center gap-1.5 transition-colors shadow-xs cursor-pointer"
          >
          ...
          <button
            ...
            className="px-5 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs shadow-xs flex items-center gap-2 transition-all hover:scale-[1.01] active:scale-[0.99] cursor-pointer"
          >
            <Save className="w-4 h-4" />
            {isEditing ? 'আপডেট সংরক্ষণ' : 'আর্টিকেল প্রকাশ করুন'}
          </button>
        </div>
      </div>
      ...
      {/* Live Preview Mode */}
      {activePreview ? (
        <div className="p-8 rounded-2xl bg-white border border-slate-200/80 shadow-xs space-y-6">
          <div className="space-y-3">
            <span className="px-3 py-1 rounded-full text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-200">
              {category}
            </span>
            <h1 className="text-2xl md:text-3xl font-bold text-slate-900 font-serifBangla">{title || 'আর্টিকেলের শিরোনাম'}</h1>
          ...
          <div className="text-sm text-slate-700 leading-relaxed font-serifBangla italic bg-slate-50 p-4 rounded-xl border-l-4 border-blue-600">
          ...
          <div className="space-y-4 pt-2 text-slate-800">
            {blocks.map((block, idx) => {
              if (block.type === 'heading') {
                return (
                  <h3 key={idx} className="text-lg font-bold text-slate-900 font-serifBangla pt-2">
                    {block.text}
                  </h3>
                );
              }
              if (block.type === 'quote') {
                return (
                  <blockquote key={idx} className="border-l-2 border-amber-400 bg-amber-50/50 p-3 rounded-r-lg italic text-slate-700">
                    "{block.text}"
                  </blockquote>
                );
              }
              if (block.type === 'list') {
                return (
                  <ul key={idx} className="list-disc list-inside space-y-1 text-slate-700 pl-2">
              ...
              if (block.type === 'link') {
                return (
                  <a key={idx} href={block.href} className="text-sm text-blue-600 underline block">
                    {block.label}
                  </a>
                );
              }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 text-sm font-semibold focus:outline-hidden focus:bg-white focus:border-blue-500 transition-colors"
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 font-mono focus:outline-hidden focus:bg-white focus:border-blue-500 transition-colors"
                  />
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-hidden focus:bg-white focus:border-blue-500 transition-colors"
                  />
                </div>
                ...
                <div className="flex items-center gap-3 pt-6">
                  ...
                  />
                  <label htmlFor="featured-checkbox" className="text-slate-700 font-semibold cursor-pointer">
          ...
          <div className="p-6 rounded-2xl bg-white border border-slate-200/80 shadow-xs space-y-4">
            ...
            <div className="space-y-4 text-xs">
              ...
              <div className="pt-2 border-t border-slate-100">
                <label className="block text-slate-700 font-medium mb-1.5">ছবির বিবরণ (Alt Text)</label>
                <input
                  type="text"
                  value={imageAlt}
                  onChange={(e) => setImageAlt(e.target.value)}
                  placeholder="যেমন: চাঁদপুরের পদ্মার তাজা রূপালী ইলিশ আহরণ"
                  className="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-hidden focus:bg-white focus:border-blue-500 transition-colors"
                />
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 text-xs flex items-center gap-1 cursor-pointer transition-colors"
                >
                ...
                >
                  ...
                  {block.type === 'heading' && (
                    <input
                      ...
                      className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-900 font-bold focus:outline-hidden focus:border-blue-500 transition-colors"
                    />
                  )}
                  ...
                  {block.type === 'paragraph' && (
                    <textarea
                      ...
                      className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-900 leading-relaxed focus:outline-hidden focus:border-blue-500 transition-colors"
                    />
                  )}
                  ...
                  {block.type === 'quote' && (
                    <input
                      ...
                      className="w-full px-3 py-2 bg-amber-50/50 border border-amber-300 rounded-lg text-slate-800 italic focus:outline-hidden focus:border-amber-500 transition-colors"
                    />
                  )}
                      ...
                      {block.items.map((item, itemIdx) => (
                        <div key={itemIdx} className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            className="flex-1 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-slate-900 focus:outline-hidden focus:border-blue-500 transition-colors"
                          />
                          <button
                            type="button"
          ...
          <div className="p-6 rounded-2xl bg-white border border-slate-200/80 shadow-xs space-y-4">
            ...
            <div className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-700 font-medium mb-1">এসইও মেটা টাইটেল</label>
                <input
                  type="text"
                  value={seoTitle}
              ...
              <div>
                <label className="block text-slate-700 font-medium mb-1">এসইও মেটা ডেসক্রিপশন</label>
                <textarea
                  rows={2}
              ...
              <div>
                <label className="block text-slate-700 font-medium mb-1">কীওয়ার্ড / ট্যাগ (কমা দিয়ে লিখুন)</label>
                <input
                  type="text"
                  value={keywords}
