...
export const GangchillSignature: React.FC<SignatureProps> = ({
  ...
  if (variant === 'vertical') {
    return (
      <div className={`flex flex-col items-center justify-center space-y-2 text-gangchill-earth/40 ${className}`}>
        <div className="w-px h-8 bg-gangchill-ink/15" />
        <span className="w-2 h-2 rotate-45 border border-gangchill-green/60" />
        <div className="w-px h-8 bg-gangchill-ink/15" />
      </div>
    );
  }
  ...
  return (
    <div className={`w-full flex items-center justify-center my-10 sm:my-16 text-gangchill-earth/40 ${className}`}>
      <div className="h-px bg-gangchill-ink/12 flex-1 max-w-xs sm:max-w-sm" />
      <div className="px-4 flex items-center gap-2">
        <svg width="32" height="12" viewBox="0 0 32 12" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-gangchill-green">
